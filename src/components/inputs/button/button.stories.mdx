...
export const argTypes = {
  ...
  iconPosition: {
    table: {
      defaultValue: { summary: defaultArgs.iconPosition},
    },
  ...
  fillWidth: {
    table: {
      defaultValue: { summary: defaultArgs.fillWidth},
    },
    control: { type: 'inline-radio'}, options: [true, false],
  },
}
...
<Meta
  ...
  decorators={[
    moduleMetadata({
      imports: [ButtonModule, MatButtonModule, PmButtonDirective]
    })
  ]}
/>
...
É possível alterar o tamanho dos botões com o input `size`. Tendo os valores `sm` ou `md`

Exemplo:

<Canvas>
  <Story name='Small' story={SmallButton} />
</Canvas>
...
<Canvas>
  <Story name='Directive' story={DirectiveButton}/>
</Canvas>
